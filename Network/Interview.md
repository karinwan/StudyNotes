...
- DNS`Domain Name System` 将人类可读的域名解析为 IP 地址。
- 解析过程包括先查**缓存**`Local Cache`，再由**本地 DNS 服务器**`DNS Resolver`向**根服务器**`Root DNS` -> **顶级域名服务器**`TLD DNS` -> **权威DNS服务器**`Authoritative DNS`依次查询，最终返回目标 IP。
- 整个过程对用户透明，但对网站访问是第一步。
...
- TCP 实现**流量控制**的核心机制。
- 发送方可以一次发送多个**未被确认**的数据包，不超过窗口大小`min(cwnd, rwnd)`，充分利用带宽提升传输效率。
- **接收窗口**`rwnd`：**接收方**告知的剩余缓冲区大小（通过 `ACK` 通知）。
- **拥塞窗口**`cwnd`：**发送方**根据**网络拥塞状态**动态调整，控制自己最多能发多少字节。实际窗口大小为`min(rwnd, cwnd)`。


#### 拥塞控制

TCP 为了 保护网络不被一开始就压垮，它在建立连接后，会从小窗口开始慢慢试探性地增加速率，直到达到带宽上限或发生丢包。

| 阶段名称       | 触发条件                     | 特点描述                             | 拥塞窗口增长方式     |
|----------------|------------------------------|--------------------------------------|----------------------|
| 慢开始`Slow Start` | 连接刚建立或丢包重传后开始     | 初始 `cwnd` 很小，随着 ACK 到达快速增长 | 指数增长（2 倍 |
| 拥塞避免`Congestion Avoidance` | `cwnd` 达到**慢开始阈值**`ssthresh` | 为避免网络拥塞，增长速率变缓 | 线性增长（+1） |
| 快重传`Fast Retransmit` | 丢包检测机制(收到连续 3 个重复 ACK) | 表示某段丢包，**无需超时立即重传**   | 不改变 `cwnd` 大小     |
| 快恢复`Fast Recovery` | 快重传后进入                   | 将 `cwnd` **减半**并继续**线性增长** | 减半后线性恢复 |


#### TCP 粘包 / 拆包

TCP 是面向**字节流**的协议，没有**消息边界**，并且为了效率，操作系统会**缓存**再发，接收端可能一次收到多个消息或不完整消息。

- **粘包**`packet sticking`：多个数据包被“粘”在一起接收；
- **拆包**`packet splitting`：一个完整数据包被拆成多次接收。

解决方式（应用层）：
- **定长协议**：每个消息固定长度（如 1024 字节）。简单高效，但可能浪费空间；
- **分隔符协议**：用特殊字符分隔（如 \n, ##）；
- **长度字段**：消息头中包含数据长度，最常用且通用（如 Protobuf、MQTT）


#### TCP如何确保稳定传输
- **三次握手**建立连接；
- 使用**序列号和ACK**保证数据有序与送达；
- **超时重传**防止丢包；
- **滑动窗口**控制流量；
- **拥塞控制**防止网络过载；
- **四次挥手**则确保数据完全传输后再断开连接。
...
- **HTTP/2：**
  - 使用**二进制**格式代替文本，更高效；
  - **多路复用**：一个连接中可同时处理多个请求和响应，避免队头阻塞；
  - **头部压缩**：请求头被压缩，减少重复信息，提升带宽利用率；
  - **服务器推送**：服务器可主动推送客户端未来可能需要的资源，减少等待时间；
  - **优先级和流量控制**：客户端可以为请求设置**优先级**，服务器按需响应，有助于资源调度。
- **HTTP/3：** 基于 `UDP/QUIC`，**内置加密**（无需额外 TLS 层），天然避免队头阻塞，性能更好、延迟更低。
...
#### 常用请求头和响应头
请求头：
- `Host`：指定请求的目标主机（必填），如 `Host: www.example.com`；
- `User-Agent`：标识客户端类型（浏览器、操作系统等）；
- `Accept`：告诉服务器能接收哪些响应内容类型；
- `Content-Type`：请求体中的数据类型（用于`POST/PUT`）；
- `Authorization`：携带身份验证信息（如`Token`）；
- `Cookie`：携带客户端`Cookie`。

响应头：
- `Content-Type`：指定响应体的类型（HTML、JSON、图片等）；
- `Cache-Control`：控制缓存行为（是否缓存、缓存多长时间等）；
- `Set-Cookie`：设置 Cookie，发送给客户端保存；
- `Location`：重定向地址（用于 3xx 响应），浏览器会自动跳转到这个新地址；
